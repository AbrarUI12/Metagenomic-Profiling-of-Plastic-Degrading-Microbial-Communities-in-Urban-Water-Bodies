# Metadata summary
- Retrieval date (UTC): 2026-01-14T08:21:52.411699Z
- Source: NCBI SRA runinfo (SRR=SRR23872596)

## Key fields
- Run: SRR23872596
- BioSample: SAMN33764824
- BioProject: PRJNA944918
- SampleName: TezpurPTM_Brahmaputra
- Sample: SRS17053977
- ScientificName: sediment metagenome
- LibraryStrategy: AMPLICON
- LibrarySource: METAGENOMIC
- LibrarySelection: PCR
- Platform: ILLUMINA
- ReleaseDate: 2024-05-01 07:18:45
- LoadDate: 2023-03-17 12:19:24
- spots: 13495134
- bases: 4291452612
- avgLength: 318

## Notes
- This summary is derived from SRA run metadata and may not include detailed environmental context beyond what submitters provide.
- Full metadata JSON: `outputs/metadata/metadata.json`
